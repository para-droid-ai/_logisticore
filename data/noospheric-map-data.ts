...
import { AI1_NAME, AI2_NAME, COMMAND_CONSOLE_ID } from '../constants'; // Added COMMAND_CONSOLE_ID
...
export function calculateInitialFactionData(mapNodes: Record<string, NoosphericNodeData>): Record<NoosphericPlayerId, NoosphericFaction> {
    const factions: Record<NoosphericPlayerId, NoosphericFaction> = {
        ...
        'NEUTRAL': { id: 'NEUTRAL', name: 'NEUTRAL', color: '#888888', qr: 0, nodesControlled: 0, totalUnits: 0, kjsHeld: 0, tacticalAnalysis: "Maintaining neutral stance.", totalStandardUnits: 0, totalEvolvedUnits: 0, activeHubsCount: 0, successfulAttacks: 0, attacksLost: 0, successfulDefenses: 0, defensesLost: 0, successfulTurnAttempts: 0, failedTurnAttempts: 0, unitsPurchased: 0, unitsLost: 0, tacticalAnalysisHistory: [] },
        'COMMAND_CONSOLE': { // Ensure COMMAND_CONSOLE has all fields of NoosphericFaction
            id: COMMAND_CONSOLE_ID as NoosphericPlayerId,
            name: 'COMMAND',
            color: '#FFFF00', // Example color, can be adjusted
            qr: 0,
            nodesControlled: 0,
            totalUnits: 0,
            kjsHeld: 0,
            tacticalAnalysis: "Overseeing operations.",
    ...
    for (const nodeId in mapNodes) {
        ...
        if (node.owner !== 'NEUTRAL' && node.owner !== COMMAND_CONSOLE_ID) { // Exclude COMMAND_CONSOLE from these calculations
        ...
        } else if (node.owner === 'NEUTRAL') { // Only calculate for NEUTRAL
