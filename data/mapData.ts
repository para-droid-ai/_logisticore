...
import { NodeData, MapType, NodeType, PlayerId } from '../types';
import { AI1_ID, AI2_ID, NEUTRAL_ID, ATTRITION_STARTING_MAT, FORT_HP_PER_LEVEL } from '../constants';
...
interface MapNodeDefinition {
  ...
  MAT_output?: number;
  maxUnits?: number;
  isKJOriginal?: boolean; // Used by Seraphim for its KJs
  isCNOriginal?: boolean; // To identify CNs for FoW setup if specific start IDs aren't available
}
...
// Interface for raw node data for maps like Twin Peaks, Classic Lattice, Tartarus Anomaly
interface MinimalRawNodeDefinition {
  id: string;
  name: string;
  type: NodeType;
  connections_orig: string[];
  mapPosition: { x: number; y: number };
  initialOwner?: PlayerId;
  initialUnits?: number;
  fortLevel?: number;
  qrOutput?: number;
  MAT_output?: number; // Added for explicitness
}


const VOLGOGRAD_CAULDRON_NODES: MapNodeDefinition[] = [
  // AXIOM West
  { id: "CN-W", name: "Western Staging", type: "CN", mapPosition: { x: 10, y: 50 }, initialOwner: AI2_ID, initialUnits: 25, labelOverride: "Western Staging", qrOutput: 10, MAT_output: 15, isCNOriginal: true, fortLevel: 1 }, // MaxUnits will be 100 by new global rule
  { id: "WSH", name: "West Supply Hub", type: "INDUSTRIAL_HUB", mapPosition: { x: 20, y: 35 }, initialOwner: AI2_ID, initialUnits: 0, fortLevel: 1, labelOverride: "West Supply Hub", qrOutput: 8, MAT_output: 20 }, // MaxUnits will be 50
  { id: "WMP", name: "West Motor Pool", type: "STANDARD", mapPosition: { x: 20, y: 65 }, initialOwner: AI2_ID, initialUnits: 0, labelOverride: "West Motor Pool", qrOutput: 5, MAT_output: 5 }, // MaxUnits will be 50
  { id: "FBD", name: "Recon Array Don", type: "RECON_ARRAY", mapPosition: { x: 30, y: 50 }, initialOwner: NEUTRAL_ID, initialUnits: 5, labelOverride: "RA-Don", qrOutput: 0, MAT_output: 0 }, // MaxUnits will be 50 (was 15 in def)
  { id: "WG", name: "West Gate", type: "STANDARD", mapPosition: { x: 40, y: 50 }, initialOwner: AI2_ID, initialUnits: 18, labelOverride: "West Gate", qrOutput: 5, MAT_output: 5 }, // MaxUnits will be 50
  { id: "BA", name: "Beketovka Approach", type: "STANDARD", mapPosition: { x: 45, y: 75 }, initialOwner: AI2_ID, initialUnits: 1, labelOverride: "Beketovka Approach", qrOutput: 5, MAT_output: 5 }, // MaxUnits will be 50
  // Neutral Center & Strongpoints
  { id: "NS", name: "Northern Strongpoint", type: "FORTRESS", mapPosition: { x: 50, y: 15 }, initialOwner: NEUTRAL_ID, initialUnits: 10, labelOverride: "Northern Strongpoint", qrOutput: 12, MAT_output: 10, fortLevel: 2 }, // MaxUnits will be 75 (was 40 in def)
  { id: "KA", name: "Kotluban Approach", type: "STANDARD", mapPosition: { x: 38, y: 25 }, initialOwner: AI2_ID, initialUnits: 1, labelOverride: "Kotluban Approach", qrOutput: 5, MAT_output: 5 }, // MaxUnits will be 50
  { id: "NB", name: "North Bridge", type: "STANDARD", mapPosition: { x: 50, y: 35 }, initialOwner: NEUTRAL_ID, initialUnits: 0, labelOverride: "North Bridge", qrOutput: 5, MAT_output: 0 }, // MaxUnits will be 50
  { id: "MK", name: "Mamayev Kurgan", type: "URBAN", mapPosition: { x: 50, y: 50 }, initialOwner: NEUTRAL_ID, initialUnits: 5, labelOverride: "Mamayev Kurgan", qrOutput: 7, MAT_output: 10 }, // MaxUnits will be 50
  { id: "TF", name: "Tractor Factory", type: "INDUSTRIAL_HUB", mapPosition: { x: 58, y: 45 }, initialOwner: NEUTRAL_ID, initialUnits: 5, labelOverride: "Tractor Factory", qrOutput: 8, MAT_output: 20 }, // MaxUnits will be 50
  { id: "OP", name: "October Plant", type: "INDUSTRIAL_HUB", mapPosition: { x: 62, y: 55 }, initialOwner: NEUTRAL_ID, initialUnits: 5, labelOverride: "October Plant", qrOutput: 8, MAT_output: 20 }, // MaxUnits will be 50
  { id: "CS", name: "Central Station", type: "STANDARD", mapPosition: { x: 55, y: 60 }, initialOwner: NEUTRAL_ID, initialUnits: 0, labelOverride: "Central Station", qrOutput: 5, MAT_output: 5 }, // MaxUnits will be 50
  { id: "SB", name: "South Bridge", type: "STANDARD", mapPosition: { x: 50, y: 68 }, initialOwner: NEUTRAL_ID, initialUnits: 0, labelOverride: "South Bridge", qrOutput: 5, MAT_output: 0 }, // MaxUnits will be 50
  { id: "SS", name: "Southern Strongpoint", type: "FORTRESS", mapPosition: { x: 50, y: 85 }, initialOwner: NEUTRAL_ID, initialUnits: 10, labelOverride: "Southern Strongpoint", qrOutput: 12, MAT_output: 10, fortLevel: 2 }, // MaxUnits will be 75 (was 40 in def)
  // GEM-Q East
  { id: "GH", name: "Gorodishche Heights", type: "STANDARD", mapPosition: { x: 65, y: 25 }, initialOwner: AI1_ID, initialUnits: 6, labelOverride: "Gorodishche Heights", qrOutput: 5, MAT_output: 5 }, // MaxUnits will be 50
  { id: "EG", name: "East Gate", type: "STANDARD", mapPosition: { x: 70, y: 50 }, initialOwner: AI1_ID, initialUnits: 9, labelOverride: "East Gate", qrOutput: 5, MAT_output: 5 }, // MaxUnits will be 50
  { id: "KR", name: "Krasnoarmeysk Ruins", type: "STANDARD", mapPosition: { x: 65, y: 75 }, initialOwner: AI1_ID, initialUnits: 3, labelOverride: "Krasnoarmeysk Ruins", qrOutput: 5, MAT_output: 5 }, // MaxUnits will be 50
  { id: "FBV", name: "Recon Array Volga", type: "RECON_ARRAY", mapPosition: { x: 80, y: 50 }, initialOwner: NEUTRAL_ID, initialUnits: 5, labelOverride: "RA-Volga", qrOutput: 0, MAT_output: 0 }, // MaxUnits will be 50 (was 15 in def)
  { id: "EMP", name: "East Motor Pool", type: "STANDARD", mapPosition: { x: 85, y: 65 }, initialOwner: AI1_ID, initialUnits: 1, labelOverride: "East Motor Pool", qrOutput: 5, MAT_output: 5 }, // MaxUnits will be 50
  { id: "ESH", name: "East Supply Hub", type: "INDUSTRIAL_HUB", mapPosition: { x: 85, y: 35 }, initialOwner: AI1_ID, initialUnits: 1, labelOverride: "East Supply Hub", qrOutput: 8, MAT_output: 20 }, // MaxUnits will be 50
  { id: "CN-E", name: "Eastern Staging", type: "CN", mapPosition: { x: 90, y: 50 }, initialOwner: AI1_ID, initialUnits: 25, labelOverride: "Eastern Staging", qrOutput: 10, MAT_output: 15, isCNOriginal: true, fortLevel: 1 }, // MaxUnits will be 100
];
const VOLGOGRAD_CAULDRON_CONNECTIONS: [string, string][] = [
...
const VOLGOGRAD_CAULDRON_DEFINITION: MapDefinition = {
...
const SERAPHIM_GRID_CONNECTIONS_RAW: [string, string][] = [
...
const SERAPHIM_GRID_NODES: MapNodeDefinition[] = SERAPHIM_GRID_NODES_RAW.map(n => {
    let logisticoreType: NodeType; let initialOwner: PlayerId = NEUTRAL_ID; let initialUnits = 0; let qrOutput = 0; let MAT_output = 0; let isKJOriginal = false; let isCNOriginal = false; let fortLevel = 0; let labelOverride = n.id;
    if (n.id === "A-4") { logisticoreType = 'RECON_ARRAY'; initialOwner = NEUTRAL_ID; initialUnits = 5; qrOutput = 0; MAT_output = 0; labelOverride = "RA-Alpha"; } // maxUnits will be 50
    else if (n.id === "B-4") { logisticoreType = 'RECON_ARRAY'; initialOwner = NEUTRAL_ID; initialUnits = 5; qrOutput = 0; MAT_output = 0; labelOverride = "RA-Beta"; } // maxUnits will be 50
    else if (n.id.includes('-CN')) { logisticoreType = 'CN'; isCNOriginal = true; initialOwner = n.id === 'A-CN' ? AI1_ID : (n.id === 'B-CN' ? AI2_ID : NEUTRAL_ID); initialUnits = 25; qrOutput = 10; MAT_output = 15; fortLevel = 1; } // maxUnits will be 100
    else if (n.id.includes('-KJ')) { logisticoreType = 'INDUSTRIAL_HUB'; isKJOriginal = true; if (["G-KJ", "D-KJ", "E-KJ"].includes(n.id)) { initialOwner = NEUTRAL_ID; initialUnits = 5; } MAT_output = 20; qrOutput = 8; } // maxUnits will be 50
    else { logisticoreType = 'QN'; initialUnits = 0; qrOutput = 15; MAT_output = 0; } // QN maxUnits will be 20 (default) or map-specific
...
const SERAPHIM_GRID_DEFINITION: MapDefinition = {
...
// --- Twin Peaks ---
const TWIN_PEAKS_NODES_RAW: MinimalRawNodeDefinition[] = [
...
const TWIN_PEAKS_NODES: MapNodeDefinition[] = TWIN_PEAKS_NODES_RAW.map(n => ({
...
const TWIN_PEAKS_CONNECTIONS = TWIN_PEAKS_NODES_RAW.flatMap(n => n.connections_orig.map(c => [n.id, c] as [string, string]));
const TWIN_PEAKS_DEFINITION: MapDefinition = {
...
// --- Classic Lattice ---
const CLASSIC_LATTICE_NODES_RAW: MinimalRawNodeDefinition[] = [
...
const CLASSIC_LATTICE_NODES: MapNodeDefinition[] = CLASSIC_LATTICE_NODES_RAW.map(n => ({
  id: n.id, name: n.name, type: n.type, mapPosition: n.mapPosition,
...
const CLASSIC_LATTICE_CONNECTIONS = CLASSIC_LATTICE_NODES_RAW.flatMap(n => n.connections_orig.map(c => [n.id, c] as [string, string]));
const CLASSIC_LATTICE_DEFINITION: MapDefinition = {
...
const TARTARUS_ANOMALY_NODES_RAW: MinimalRawNodeDefinition[] = [
...
const TARTARUS_ANOMALY_NODES: MapNodeDefinition[] = TARTARUS_ANOMALY_NODES_RAW.map(n => ({
  id: n.id, name: n.name, type: n.type, mapPosition: n.mapPosition,
  initialOwner: n.initialOwner, initialUnits: n.initialUnits, qrOutput: n.qrOutput,
  labelOverride: n.name, MAT_output: n.MAT_output, fortLevel: n.fortLevel || 0 // MaxUnits will be determined by global rules
}));
const TARTARUS_ANOMALY_CONNECTIONS = TARTARUS_ANOMALY_NODES_RAW.flatMap(n => n.connections_orig.map(c => [n.id, c] as [string, string]));
const TARTARUS_ANOMALY_DEFINITION: MapDefinition = {
...
export function getMapData(mapType: MapType, isFogOfWarActive: boolean): Record<string, NodeData> {
  ...
  const transformedNodes: Record<string, NodeData> = {};
  ...
  if (mapDef.viewBox) {
    ...
    if (parts.length === 4 && !parts.some(isNaN)) {
      ...
      if (viewBoxWidth <= 0) viewBoxWidth = 100; // Default width if invalid
      if (viewBoxHeight <= 0) viewBoxHeight = 100; // Default height if invalid
    } else {
      console.warn(`Invalid viewBox format: "${mapDef.viewBox}". Defaulting to "0 0 100 100".`);
      minX = 0; minY = 0; viewBoxWidth = 100; viewBoxHeight = 100;
    }
  }


  mapDef.connections.forEach(([a, b]) => {
    if (!allConnections[a]) allConnections[a] = [];
    if (!allConnections[b]) allConnections[b] = [];
    if (!allConnections[a].includes(b)) allConnections[a].push(b);
    if (!allConnections[b].includes(a)) allConnections[b].push(a);
  });

  mapDef.nodes.forEach(nodeDef => {
    let finalOwner: PlayerId;
    let finalStandardUnits: number;
    let finalFortLevel: number;

    if (isFogOfWarActive) {
        finalOwner = NEUTRAL_ID;
        ...
        if (nodeDef.type === 'RECON_ARRAY') {
            ...
            const significantNeutralTypes: NodeType[] = ['INDUSTRIAL_HUB', 'FORTRESS', 'URBAN', 'CN', 'QN']; 
            if ((nodeDef.initialUnits && nodeDef.initialUnits > 0) &&
                (significantNeutralTypes.includes(nodeDef.type) || (mapType === MapType.SERAPHIM_GRID && nodeDef.isKJOriginal))) {
                finalOwner = NEUTRAL_ID;
        ...
        finalOwner = nodeDef.initialOwner || NEUTRAL_ID;
    ...
    const maxHP = finalFortLevel * FORT_HP_PER_LEVEL;

    let newMaxUnitsValue: number;
    if (nodeDef.type === 'CN') {
